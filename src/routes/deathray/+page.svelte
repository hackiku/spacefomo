<script>
    ...
    function calculateTemperature(heatFl, focalSize) {
        ...
        const energyAbsorbed = heatFl * focalPointArea * 3600; // Exposure time = 3600 seconds
        return BASE_TEMPERATURE + energyAbsorbed / (regolithMass * SPECIFIC_HEAT_CAPACITY);
    }

    // Function to update calculations
    function updateCalculations() {
        if (!$lockMirrorDiameter) {
            mirrorDiameter.set(calculateMirrorDiameter($focalPointSize, $orbitalAltitude));
        }
        if (!$lockFocalPointSize) {
            // Example logic: Inversely proportional to altitude, proportional to diameter
            focalPointSize.set($mirrorDiameter / Math.sqrt($orbitalAltitude));
        }
        if (!$lockTemperature) {
            temperature.set(calculateTemperature($heatFlux, $focalPointSize));
        }
        // Always update heat flux
...
<div class="bg-gray-900 p-6 m-32 rounded-xl text-white">
    ...
    <div class="bg-gray-800 p-4 rounded-lg mb-6">
        <div class="flex justify-between items-center" class:opacity-60={$lockOrbitalAltitude}>
            <label for="orbitalAltitudeSlider" class="block text-lg mb-2">
                Orbital Altitude (0 - 300,000 meters):
            </label>
            <button on:click={() => toggleLock(lockOrbitalAltitude)}>
