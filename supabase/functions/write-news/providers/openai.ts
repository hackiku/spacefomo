// supabase/functions/write-news/providers/openai.ts
...
Here's the article URL: ${url}`;
	...
	private async callWithRetries(systemPrompt: string, userMessage: string, plaintext: string | null): Promise<ExtractedData> {
		...
		for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
			try {
				...
				// Parse JSON - should be direct since we used json_object response format
		...
		throw lastError || new Error('Failed to call OpenAI API after all retries');
	}
	...
	private normalizeData(data: any, plaintext: string | null): ExtractedData {
		...
		if (data.title && typeof data.title === 'string') {
			...
			normalized.impact_score = data.impact_score || 70;
